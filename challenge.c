...
// =========================================================
// FUNCTION PROTOTYPE (Fixed naming mismatch)
// =========================================================
 ...
 * sparse_multiply
 * 
 * Performs a single-pass traversal of a row-major dense matrix A to:
 * 1. Extract non-zero elements into CSR format (values, col_indices, row_ptrs).
 * 2. Compute the matrix-vector product y = A * x.
 * 
 * Constraints: Zero dynamic memory allocation.
 */
void sparse_multiply(
    int rows,
    int cols,
    ...
    for (int i = 0; i < rows; i++) {
        ...
        for (int j = 0; j < cols; j++) {
            ...
            if (val != 0.0) {
                ...
                // Accumulate product for the output vector y
                sum += val * x[j];
        ...
        // Store result in y and update row pointer
        y[i] = sum;
        row_ptrs[i + 1] = nnz;
    }
...
int main(void) {
    ...
    for (int iter = 0; iter < num_iterations; ++iter) {
        ...
        // Reference dense multiplication
        for (int i = 0; i < rows; ++i) {
            double sum = 0.0;
            for (int j = 0; j < cols; ++j) {
                sum += A[i * cols + j] * x[j];
